...
typedef struct DW5_32_CompilationUnitHeader {
    uint32_t unit_length;
    uint16_t version;
    uint8_t  unit_type;
    uint8_t  address_size;
    uint32_t debug_abbrev_offset;
} DW5_32_CompilationUnitHeader;

typedef struct DW5_32_LineNumberProgramHeader {
    uint32_t unit_length;
    uint16_t version;
    uint8_t address_size;
    ...
    // uint8_t directories_count;
    // char directories[]; // sequence of directory names
    // uint8_t file_name_entry_format_count;
    // uint8_t file_name_entry_format[]; // ULEB128 pairs
    // uint8_t file_names_count;
    // uint8_t file_names[]; // sequence of file name entries
} DW5_32_LineNumberProgramHeader;

typedef struct ArangesHeader {
    uint32_t unit_length;
    uint16_t version;
...
typedef struct CommonInformationEntry {
    uint32_t length; // must be a multiple of 8 (or the address of the target system)
    uint32_t CIE_id;
    uint8_t version;
    ...
    // initial_instructions, array of uint8_t
    // padding of DW_CFA_nop to match the specified length
} CommonInformationEntry;

static const int SIZE64 = 24;
static const int SIZE32 = 16;
typedef struct FrameDescriptionEntry {
    uint32_t length; // multiple of 8?
    uint32_t CIE_pointer; // points to common information entry this description belongs to?
    union {
        struct {
        ...
        struct {
            uint32_t initial_location32;
            uint32_t address_range32;
        };
    };
    // instructions, array of uint8_t
...
int ULEB128_encode(uint8_t* buffer, uint32_t buffer_space, uint64_t value);
uint64_t ULEB128_decode(uint8_t* buffer, uint32_t buffer_space);
int SLEB128_encode(uint8_t* buffer, uint32_t buffer_space, int64_t value);
int64_t SLEB128_decode(uint8_t* buffer, uint32_t buffer_space);
void LEB128_test();
...
#define DW_UT_compile 0x1
#define DW_UT_type 0x2
#define DW_UT_partial 0x3
#define DW_UT_skeleton 0x4
#define DW_UT_split_compile 0x5
#define DW_UT_split_type 0x6
